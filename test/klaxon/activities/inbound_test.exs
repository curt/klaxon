defmodule Klaxon.Activities.InboundTest do
  ...
  describe "`Klaxon.Activities.Inbound.request_well_formed?`" do
    test "with all good arguments" do
      assert Inbound.request_well_formed?(@good_payload, @good_req_headers)
    end

    test "with `payload` not map" do
      refute Inbound.request_well_formed?(1, @good_req_headers)
    end

    test "with `payload` nil" do
      refute Inbound.request_well_formed?(nil, @good_req_headers)
    end

    test "with `req_headers` not list" do
      refute Inbound.request_well_formed?(@good_payload, 1)
    end

    test "with `req_headers` empty list" do
      refute Inbound.request_well_formed?(@good_payload, [])
    end

    test "with `req_headers` nil" do
      refute Inbound.request_well_formed?(@good_payload, nil)
    end

    test "with missing `actor` in payload" do
      refute Inbound.request_well_formed?(
