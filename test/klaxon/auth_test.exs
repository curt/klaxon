defmodule Klaxon.AuthTest do
    ...
    test "sends token through notification", %{user: user} do
      token =
        extract_user_token(fn url ->
          Auth.deliver_update_email_instructions(user, "current@example.com", url, sender())
        end)
  ...
  describe "update_user_email/2" do
    setup do
      ...
      token =
        extract_user_token(fn url ->
          Auth.deliver_update_email_instructions(%{user | email: email}, user.email, url, sender())
        end)
  ...
  describe "deliver_user_confirmation_instructions/2" do
    ...
    test "sends token through notification", %{user: user} do
      token =
        extract_user_token(fn url ->
          Auth.deliver_user_confirmation_instructions(user, url, sender())
        end)
  ...
  describe "deliver_user_reset_password_instructions/2" do
    ...
    test "sends token through notification", %{user: user} do
      token =
        extract_user_token(fn url ->
          Auth.deliver_user_reset_password_instructions(user, url, sender())
        end)
