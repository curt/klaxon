...
def parse(file_path: str) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        tags = [
            el.text for el in item.findall("category")
        ...
        attachments = [
            a for a in raw_attachments if a["post_parent"] == post_id
        ]

        normalized_content, lat, lon = normalize_content(raw_content, attachments_by_url)
...
def normalize_content(html: str, attachments_by_url: Dict[str, Dict]) -> tuple[str, Optional[float], Optional[float]]:
    html = unescape(html)
...
def _extract_caption(item: ET.Element) -> Optional[str]:
    # In some themes, caption may be inside content
    content_el = item.find("content:encoded", NAMESPACES)
    if content_el is not None:
        match = re.search(r'\[caption.*?\](.*?)\[/?caption\]', content_el.text or "", re.DOTALL)
        if match:
            inner = match.group(1)
...
def _replace_caption_shortcodes(text: str) -> str:
    caption_re = re.compile(
        r'\[caption [^\]]*?\](.*?)<img ([^>]+)>.*?<br\s*/?>\s*(.*?)\[/caption\]',
        re.IGNORECASE | re.DOTALL
    )
    def repl(m): return f"<figure><img {m.group(2)}><figcaption>{m.group(3).strip()}</figcaption></figure>"
    return caption_re.sub(repl, text)


def _replace_embed_shortcodes(text: str) -> str:
    embed_re = re.compile(r'\[embed\](.*?)\[/embed\]', re.IGNORECASE)
    return embed_re.sub(lambda m: f'<div class="embed"><a href="{m.group(1).strip()}">{m.group(1).strip()}</a></div>', text)


def _remove_gallery_shortcodes(text: str) -> str:
    return re.sub(r'\[gallery[^\]]*\]', '', text, flags=re.IGNORECASE)


def _remove_comment_blocks(text: str) -> str:
    return re.sub(r'<!--.*?-->', '', text, flags=re.DOTALL)


def _consolidate_line_feeds(text: str) -> str:
    # Replace multiple newlines with a single one
    return re.sub(r'\n{2,}', '\n', text)


def _extract_and_remove_leaflet_shortcode(text: str) -> tuple[str, Optional[float], Optional[float]]:
    pattern = re.compile(
        r'\[leaflet-map\b([^]]*)\]',
        re.IGNORECASE
    )
...
def rewrite_image_url(url: str) -> str:
    return url  # Replace with your CDN or attachment logic


def rewrite_url(url: str) -> str:
    return url  # Rewrite internal links if needed


def _sanitize_html(soup: BeautifulSoup):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python wp_export_parser.py export.xml")
        sys.exit(1)
