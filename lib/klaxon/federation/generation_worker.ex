defmodule Klaxon.Federation.GenerationWorker do
  ...
  def perform(%{
    ...
    Activities.send_object(schema, actor, object, action, follower)
    :ok
  end
  ...
  def perform(%{
        args:
          %{"schema" => schema, "actor" => actor, "object" => object, "action" => action} = args
      }) do
    Logger.info("Generate distribution workers: #{inspect(args)}")

    for follower <- Klaxon.Federation.get_follower_uris(actor) do
      %{:schema => schema, actor: actor, object: object, action: action, follower: follower}
